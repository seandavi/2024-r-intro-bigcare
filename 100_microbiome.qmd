...
In this chapter, we will assume that the data has already been processed and cleaned, and we will focus on analyzing the microbiome data using R. We will explore common microbiome analysis techniques, including alpha and beta diversity analysis, and visualizing microbiome data.
...
Now that the `mia` package is loaded, we can load the microbiome dataset that we will be working with. In this case, we will use the `Tito2024QMP` dataset, which contains microbial abundance data for a set of colorectal cancer samples, colon adenoma samples, and health controls. You can look at the details of the dataset using `help('Tito2024QMP')`. 

To load the dataset into your R session, you can use the following command:

```{r}
# Load the Tito2024QMP dataset
data("Tito2024QMP")
```

The `Tito2024QMP` dataset is now loaded into your R session and is ready for analysis. We can double-check that the dataset has been loaded correctly by examining its structure and contents.

```{r}
# Bioconductor datasets are typically stored as SummarizedExperiment objects
# typing the object name will show some basic information about the dataset
Tito2024QMP
```

When looking at the output of the `Tito2024QMP` object, you will see information about the number of samples, the number of features (microbes), and additional information about what is stored with the dataset. 
...
Once the data is loaded, we can start exploring the microbiome dataset. The `Tito2024QMP` dataset is a `TreeSummarizedExperiment` object, a common data structure used in Bioconductor for storing high-throughput biological data. It contains information about the microbial abundances in different samples, as well as metadata associated with the samples.

To reduce the number of keys that we need to type and to make the code more readable, we can assign the `Tito2024QMP` object to a shorter variable name, such as `tse`:

```{r}
# Assign the Tito2024QMP dataset to a shorter variable name
tse <- Tito2024QMP
```

This effectively made a copy of the `Tito2024QMP` object and stored it in the `tse` variable. We can now use the `tse` variable to refer to the dataset throughout the rest of the analysis.
